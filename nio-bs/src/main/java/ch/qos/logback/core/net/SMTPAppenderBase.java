...
public abstract class SMTPAppenderBase<E> extends AppenderBase<E> {
    ...
    private Session session;// 每次发邮件时都是用相同的session

    /**
     * Send the contents of the cyclic buffer as an e-mail message.
     */
    protected void sendBuffer(CyclicBuffer<E> cb, final E lastEventObject) {
        ...
        try {
            ...
            // 2011-12-29 修复logback1.0.0 bug:不能正常清理cb，异步放在这里就不会有问题了，另外 MimeMessage也需要每次新建
            context.getExecutorService().execute(new Runnable() {

                @Override
                public void run() {
                    MimeBodyPart part = new MimeBodyPart();
                    MimeMessage mimeMsg = new MimeMessage(session);// 每次要新建才不会有线程安全问题
                    try {
                        if (from != null) {
                            mimeMsg.setFrom(getAddress(from));
                        } else {
                        ...
                        if (subjectLayout != null) {
                            mimeMsg.setSubject(subjectLayout.doLayout(lastEventObject), charsetEncoding);
                        }
    ...
    /**
     * The <b>EventEvaluator</b> option takes a string value representing the name of the class implementing the {@link EventEvaluator} interface. A corresponding object will be instantiated and
     * assigned as the event evaluator for the SMTPAppender.
     */
